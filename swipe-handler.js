...
function setValue(value) {
  ...
  value = Math.max(0, value - DEADZONE);
...
window.addEventListener("wheel", (e) => {
  ...
  if (currentTime - lastScrollTime < NEW_TIMEOUT) {
...
function step() {
  ...
  if (!userScrolled && scrollAmountThisFrame != 0) {
    holding = true;
    animationSlideAmount += scrollAmountThisFrame * THRESHOLD * 0.0075;
  ...
  let timedOut = currentTime - lastMoveTime > NEW_TIMEOUT;
  if (timedOut) {
    holding = false;
  }
